...
const activeSessionsSocket = (io, dbConnection) => {
    const users = [];
    const indexStudyPrograms = [68,69,70,76,77,80,81,82,90];
    ...
    io.on('connection', async (socket) => {
        ...
        if (users[user.id_moodle].activeExams.filter(a => a.tipo == "2").length > 1) {
